...
public class ResponseUtil {
    ...
    public static void sendMessage(SlingHttpServletResponse response, String key, String value) throws IOException {
        sendContent(response, getJsonMessage(key, value));
    }

    public static void sendError(SlingHttpServletResponse response, String value) throws IOException {
        sendContent(response, HttpServletResponse.SC_BAD_REQUEST, getJsonMessage("error", "Invalid request"));
    }

    public static void sendContent(SlingHttpServletResponse response, String content) throws IOException {
        sendContent(response, HttpServletResponse.SC_OK, content);
    }

    private static void sendContent(SlingHttpServletResponse response, int status, String content) throws IOException {
